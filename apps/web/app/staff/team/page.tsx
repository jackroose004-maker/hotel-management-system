...
function StaffModal({ member, onClose, onSaved }: {
  ...
  // password/confirmPw only used for edit (reset password); create flow sends no password — backend auto-generates
    ...
    try {
      const { data } = isNew
        ? await api.post('/users/staff',
            { name: name.trim(), email: email.trim(), role: derivedRole, staffRoleId: staffRoleId ?? undefined },
    ...
    <ModalBackdrop onClick={onClose}>
      <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden" style={{ backgroundColor: 'var(--card-bg)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          {/* Password — create flow: auto-generated by backend; edit flow: optional reset */}
          {!isNew && (
            <div>
...
export default function TeamPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const msg = e?.message ?? ''
      setError(msg.toLowerCase().includes('forbidden') || msg.includes('403') ? '403' : (msg || 'Failed to load team'))
    }
        ...
        <div className="p-4 sm:p-6 flex-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4 text-center px-6">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ backgroundColor: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.18)' }}>
                <Shield size={28} className="text-red-400" />
              </div>
              <div>
                <p className="text-base font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>Access Restricted</p>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>You don't have permission to view the team list. Contact your owner to grant access.</p>
              </div>
            </div>
          ) : error ? (
