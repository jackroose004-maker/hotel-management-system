...
export interface CartItem {
  cartKey: string // `${menuItemId}_${optionIds.join('_') || 'plain'}`
  menuItemId: string
  name: string
...
// Pricing logic ported verbatim from apps/web/store/cart.ts — must stay in sync so
// mobile and web never disagree on a guest's total.
  ...
  persist(
    (set, get) => ({
      ...
      updateNotes: (key, notes) =>
        set((state) => ({ items: state.items.map((i) => (i.cartKey === key ? { ...i, notes } : i)) })),
